...
class Camera360G(Camera):
    ...
    def __init__(self, root=None, addr=None, logger=logger):
        super().__init__()
        self._root = (
            root  # root folder where to store photos from this specific camera instance
        )
    ...
    def capture_until(self, timeout=1.0):
        """
        ...
        while (_take) and (time.time() - start_time < timeout):
            try:
                ...
                self.src_photo_fn = self.capture(gp.GP_CAPTURE_IMAGE)
                dt = time.time() - start_time
                _take = False
                self.logger.info(
                    f"Picture taken in {str(self.src_photo_fn)} within {dt*1000} ms"
                )
            except:
                n += 1
        if _take:
