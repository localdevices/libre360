...
class Camera360Serial(SerialDevice):
    ...
    def __init__(self, port, logger=logger, **kwargs):
        super().__init__(
            port, logger=logger, **kwargs
        )  # baud_rate=baud_rate, timeout=timeout, parent=parent, wildcard=wildcard,
    ...
    def init(self, timeout=1.0):
        ...
        return self.success(
            "Physical camera initialized", "Physical camera initialize failed"
        )
